  ...
  <body class="flex min-h-screen bg-gray-50 text-gray-900">
    ...
    <aside id="sidebar" class="fixed md:static top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-sm shadow-lg flex flex-col transform -translate-x-full md:translate-x-0 transition-all duration-300 z-50 border-r border-gray-100">
      ...
      <div class="p-5 flex items-center justify-between border-b border-gray-100">
        <div class="flex items-center">
          <div class="h-10 w-10 rounded-xl bg-gradient-to-br from-pink-300 to-pink-500 flex items-center justify-center shadow">
            <img src="{{ asset('img/logo.jpeg') }}" alt="Logo" class="w-10 h-10 rounded-full shadow">
          </div>
      ...
      <div class="px-5 py-4 flex items-center border-b border-gray-100">
        <img src="{{ Auth::user()->avatar ?? 'https://via.placeholder.com/64' }}" alt="User" class="w-10 h-10 rounded-full ring-2 ring-pink-200 object-cover">
        <div class="ml-3 min-w-0">
          <p class="font-semibold text-gray-800 truncate">{{ Auth::user()->name }}</p>
          <p class="text-xs text-gray-500 truncate">{{ ucfirst(Auth::user()->role ?? 'admin') }}</p>
        </div>
      </div>
      ...
      <nav class="flex-1 overflow-y-auto">
        <ul class="py-3 space-y-1">
          ...
          <li>
            <a href="{{ route('admin.index') }}" class="sidebar-item group flex items-center px-4 py-2.5 text-gray-700 hover:bg-pink-50 rounded-lg mx-2 {{ request()->routeIs('admin.index') || request()->routeIs('productos.*') ? 'active' : '' }}">
              <span class="h-8 w-8 rounded-md bg-pink-100 text-pink-600 grid place-items-center mr-3">
      ...
      @auth
        <!-- Logout -->
       <form method="POST" action="{{ route('logout') }}" class="px-4 py-4 border-t border-gray-100">
                @csrf
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <header class="bg-white/90 backdrop-blur-sm shadow-sm z-10 border-b border-gray-100">
        <div class="flex items-center justify-between px-6 py-3">
          ...
          <!-- Search -->
          <form action="{{ route('admin.index') }}" method="GET" class="relative mx-4 flex-1 max-w-md">
            <input type="hidden" name="category" value="{{ request('category','todos') }}">
            <i class="fas fa-search absolute left-3 top-3 text-gray-400"></i>
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Buscar productos..." class="w-full pl-10 pr-12 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-pink-200 focus:border-transparent">
